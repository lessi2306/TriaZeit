...
begin
  ...
  try
    ...
    begin
      // Snr länger als 4 verhindern, sonst alles zulassen
      if RfidModus and not RfidRB.Checked and // Wechsel zum Snr-Modus: Zeichenlänge prüfen
      ...
      begin
        ...
        if not SnrGueltig then // Zeichenlänge > 4
          if TriaMessage(Self,'Der Eingabe-Modus wurde von RFID-Code in Startnummer geändert.'+#13+
                         'Ungültige Startnummern werden gelöscht.',
          ...
          begin
            ...
            for i:=TztColl.Count-1 downto 0 do // ungültige + LeerItems löschen
              if (Length(TztColl[i].SnrRfid) > 4) or
                 (Length(TztColl[i].SnrRfid) > 0)and(StrToIntDef(TztColl[i].SnrRfid,0)=0) then
      ...
      begin
        SnrRfidEdit.Text := ''; // immer zurücksetzen
        RfidModus := RfidRB.Checked;
        if RfidModus then // sonst nicht ändern
        begin
      ...
      for i:=0 to TztColl.Count-1 do
        ...
        else // ungültige Snr vorher gelöscht
    ...
    begin
      ...
      begin
        ...
        if TriaMessage(Self,'Zeitformat wirklich auf ' + S1 + ' umstellen?' + #13 +
                       'Bei allen bereits erfassten Zeiten werden die '+ S2 + ' endgültig gelöscht.',
                       mtConfirmation,[mbOk,mbCancel]) <> mrOk then
        begin
    ...
    if AutoSpeichernCB.Checked then // Bereichsprüfung
    begin
      if not AutoSpeichernEdit.ValidateEdit then Exit;
    ...
    if (TimerModus <> zmUhrZeit) and not StoppUhrCB.Checked then
      if (TztHauptDlg.TztGrid.ItemCount=0) or // gibt es nicht
         ...
         (TriaMessage(Self,'Es wurden bereits Zeiten im Stoppuhr-Modus erfasst.'+#13+
                      'Möchten Sie trotzdem zum Uhrzeit-Modus wechseln?',
                      mtConfirmation,[mbOk,mbCancel]) = mrOk) then
      begin
    ...
    if (TimerModus = zmUhrZeit) and StoppUhrCB.Checked then
      if (TztHauptDlg.TztGrid.ItemCount=0) or // gibt es nicht
         ...
         (TriaMessage(Self,'Es wurden bereits Zeiten im Uhrzeit-Modus erfasst.'+#13+
                      'Möchten Sie trotzdem zum Stoppuhr-Modus wechseln?',
                      mtConfirmation,[mbOk,mbCancel]) = mrOk) then
      begin
