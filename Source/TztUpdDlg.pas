...
uses
  ...
  IdAntiFreezeBase, IdAntiFreeze, IdHTTP, IdStack,//IdSSLOpenSSL,geht nicht wegen SSL-protokollfehler
...
uses AllgObj,AllgFunc,TztDat,VistaFix,TztMain;
...
begin
  ...
  UpdateDialog := TUpdateDialog.Create(TztHauptDlg);
  try
    with UpdateDialog do
    begin
      if UpdateModus = umAuto then
        TztHauptDlg.StatusBar.Panels[0].Text  := ' Überprüfe ob Internet-Update verfügbar ist...';
      TztHauptDlg.Refresh;
...
begin
  ...
  begin
    //IOHandler         := LHandler;
    IOHandler         := nil;
    Request.Referer   := Application.Title+'_'+ProgVersion.Nr;
    Request.UserAgent := Application.Title+'/'+ProgVersion.Nr+'/Update';
  ...
  VistaFix.SetzeFonts(TitelLabel.Font);
  VistaFix.SetzeFonts(Label1a.Font);
  VistaFix.SetzeFonts(Label2a.Font);
  VistaFix.SetzeFonts(Label3.Font);
  case UpdateModus of
...
begin
  ...
  case Status of
    ...
    begin
      ...
      Label1.Caption := 'Momentan verwenden Sie: ';
      Label1a.Caption := Application.Title + ' - ' + ProgVersion.Nr;
      Label2.Hide;
      Label2a.Hide;
    ...
    begin
      ...
      Label2.Show;
      Label2a.Caption := FPadProgName + ' - ' + FPadProgVersion;
      Label2a.Show;
      Label3.Font.Color := clWindowText;
      Label3.Font.Style := [];
      Label3.Caption := 'Klicken Sie auf "Weiter", um  ' +
                         FPadProgName + ' - ' + FPadProgVersion +
    ...
    usDownLoad:
    begin
      TitelLabel.Caption := FPadProgName + ' - ' + FPadProgVersion +
                           '   downloaden';
      ...
      Label3.Font.Style := [];
      Label3.Caption := 'Die Datei '+SysUtils.ExtractFileName(FZielDatei) +
                        ' wird gespeichert...';
    ...
    begin
      ...
      Label4.Caption := 'Klicken Sie auf "Weiter", um  ' +
                         Application.Title + ' - ' + ProgVersion.Nr +
                        '  zu beenden und ';
      Label4.Show;
      Label5.Caption := FPadProgName + ' - ' + FPadProgVersion +
                        '  zu installieren.';
      Label5.Show;
      WeiterBtn.Enabled := true;
    end;
...
begin
  try
    ...
    if AnsiContainsStr(FPadProgName, Application.Title) and // Tria 2006/TriaZeit
       AnsiSameStr(FPadProgVersion,ProgVersion.Nr) then SetStatus(usUpToDate)
    else
    begin // ProgName ermitteln, Länge bleibt gleich
      FPadProgName := Copy(FPadProgName,1,Length(Application.Title));
      // URL DownloadDatei
...
begin
  ...
  try
    ...
    case Status of
      ...
      begin
        // CSIDL_PERSONAL (Eigene Dateien) liefert C:\, weil verschoben auf F:
        // Default auf Desktop:
...
begin
  ...
  if ((Status=usDownload) or (Status=usPadLesen) or (Status=usNotUpToDate) or
     ...
     (TriaMessage(Self,'Das Update ist nicht abgeschlossen. Wenn Sie jetzt abbrechen, '+#13+
