...
//==============================================================================
begin
  if Assigned(Grid) and (Grid.Liste.Count > 0){(Grid.Items.Count > 0)} then
  with Grid do
    case Key of
      VK_UP:
      begin
...
begin
  ...
  for i:=0 to Liste.Count-1 do
    if (LookUpMode = lmShowAll) or // alle Items
       ...
       (Trim(Edit.Text) = Copy(Liste[i],1,Length(Trim(Edit.Text)))) then
...
begin
  ...
  // UpDown-Position setzen, bei ungültiger Text wird Min oder Max gesetzt
  // auch wenn Edit disabled
  if Assigned(FUpDown) and not FUpDown.Aendernd then
  begin
    Aendernd := true;
...
//==============================================================================
procedure TTriaUpDown.ChangingEx(Sender:TObject; var AllowChange:Boolean;
                                 NewValue:SmallInt; Direction: TUpDownDirection);
//==============================================================================
begin
 // in TCustomUpDown.CanChange wird Direction vorher gesetzt: updNone wenn NewValue ungültig

 if Assigned(FEdit) and not FEdit.Aendernd then // Änderung durch UpDown
   // ungültige Edit-Werte korrigieren
   if StrToIntDef(FEdit.Text,0) < Min then
   begin
     if Direction=updUp then
       FEdit.Text := IntToStr(Min); //in FEdit.Change wird auch Position neu gesetzt
      AllowChange := false;
    end
    else
    if StrToIntDef(FEdit.Text,0) > Max then
    begin
      if Direction=updDown then
        FEdit.Text := IntToStr(Max); //in FEdit.Change wird auch Position neu gesetzt
      AllowChange := false;
    end;
end;
