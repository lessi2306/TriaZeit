...
begin
  ...
  try
    ...
    if IOFehler or Voll or (SnrNameBufListe.Count = 0) then // Fehler beim Laden der Datei
    begin
      StartlisteDateiName := '';
      SpalteTln := false;
      if Pfad = '' then // nicht TztConfig
        if IOFehler then
          if ZeileNr = 0 then
            ...
            TriaMessage('Fehler beim Lesen von Zeile '+IntToStr(ZeileNr)+' der Datei  "'+SysUtils.ExtractFileName(DatName)+'".'+#13+
                        'Es wurden keine Startnummern importiert.',
                         mtInformation,[mbOk])
        else
        if Voll then
...
begin
  ...
  begin
    ...
    for i:=TrennPos+1 to Length(Zeile) do
...
begin
  ...
  Result := Copy(Zeile,i+1,Length(Zeile)-i);
end;
//------------------------------------------------------------------------------
...
begin
  ...
  try
    ...
    begin
      ...
      ZeitStr    := GetZeitStr;
      if RfidModus then
        if Length(SnrRfidStr) > cnRfidZeichenMax then
        begin
