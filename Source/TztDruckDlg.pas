...
pcCollation   : Der Benutzer kann angeben, ob die Kopien sortiert werden sollen.
  ...
  if TztColl.SortItems[TztColl.SortCount-1].InitWert then
    i := TztColl.SortCount-1
  else i := TztColl.SortCount;
  if i MOD Spalten <> 0 then
    MaxZeilen := i DIV Spalten + 1
  else MaxZeilen := i DIV Spalten;
...
begin
  with TztColl do
  if (Indx > 0) and (Indx < SortCount) or
     (Indx = SortCount) and not SortItems[SortCount-1].InitWert then
    if RfidModus and (RfidZeichen<4) then
      Result := Format('%4u. %4s %s',[IndexOf(SortItems[Indx-1])+1,
...
begin
  if RFIDModus then
  begin
    S1 := 'RFID'; // Spalte mindestens 4 Zeichen
    while Length(S1) < RfidZeichen do
      S1 := S1 + ' ';
  end else
    S1 := ' Snr';
  case ZeitFormat of
